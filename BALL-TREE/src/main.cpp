...
int main() {
    ...
    std::cout << "KNN search time:   " << std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()
              << " ns" << std::endl;
    ...
    start = std::chrono::steady_clock::now();
    std::vector<std::string> neighborsC = ballTree.findConstrainedNearestNeighbors(querySong, range, k, treeSongIndex);
    end = std::chrono::steady_clock::now();

    std::cout << "\nConstrained NN search time: " << std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() << " ns" << std::endl;

    std::cout << "\nK = " << k << " Nearest Neighbors within range " << range << " of song \"" << querySong << "\":" << std::endl;
    for (const std::string& neighbor : neighborsC) {
        std::cout << neighbor << std::endl;
    }
